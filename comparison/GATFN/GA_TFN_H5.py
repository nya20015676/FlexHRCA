...
from hrcStation_normGA import Human, Robot, Productt, HrcStation
from fuzzyNumber import Triangular, SVTNN, FuzzyNumber
...
def printable(record_list):
    ...
    for record in record_list:
        ...
        if not isinstance(optimal_fitness, FuzzyNumber):
...
path_list = ["/home/amtc/Desktop/optimal/comparison/GATFN/H5/H5R1/",
             "/home/amtc/Desktop/optimal/comparison/GATFN/H5/H5R2/",
             "/home/amtc/Desktop/optimal/comparison/GATFN/H5/H5R3/",
             "/home/amtc/Desktop/optimal/comparison/GATFN/H5/H5R4/",
             "/home/amtc/Desktop/optimal/comparison/GATFN/H5/H5R5/",]

Time_list = [TIME_TABLE.Time_H5R1, TIME_TABLE.Time_H5R2, 
            #  TIME_TABLE.Time_H5R3, TIME_TABLE.Time_H5R4, TIME_TABLE.Time_H5R5
             ]
...
for i in [6,7,8,9,10]:
    for j, Time in enumerate(Time_list):
        ...
        Time = transTime_SVTNN2fuzzy(Time)
        station_TFN.set_operator_product_Time([human1, human2, human3, human4, human5], robots_list[j], [product1, product2, product3], [Time, Time, Time])
        
        if __name__ == '__main__':
            optimal_fitness, mem_SVTNN_CHS_list, mem_SVTNN_CHS_fitness_list,  record_list = station_TFN.GA(pop_size=100, p_crossover=0.85, p_cross_machine=0.67, p_mutation=0.1, p_mutation_machine=0.83, max_iteration=1500, tSize=4)
            # optimal_fitness, mem_SVTNN_CHS_list, mem_SVTNN_CHS_fitness_list,  record_list = station_TFN.GA_initialed_pop(mem_static_CHS_list, pop_size=300, p_crossover=0.9, p_cross_machine=0.67, p_mutation=0.1, p_mutation_machine=0.83, max_iteration=300, tSize=4)
            
            printable(record_list)
            excel_name = path_list[j] + "{}-{}.xlsx".format(i, optimal_fitness.mean if isinstance(optimal_fitness, FuzzyNumber) else optimal_fitness)
            data_name = path_list[j] + "{}-{}.pickle".format(i, optimal_fitness.mean if isinstance(optimal_fitness, FuzzyNumber) else optimal_fitness)
