...
class FuzzyNumber:
    ...
    def __init__(self, l_std, mean, r_std, min=float('-inf'), l_confidence = 0.997, r_confidence = 0.997):
        if l_std != None and r_std != None:
            ...
            l_quantile = (1 - l_confidence)/2 # Left Quantile
            r_quantile = 1-(1-r_confidence)/2 # Right Quantile
            self.min = min
    ...
    def sampling(self):
        if self.samples != []:
            return self.samples
        # Define the original probability distribution function
...
class Triangular(FuzzyNumber):
    ...
    def maximum(triangular_list):
        max_mean = max(triangular_list, key=lambda x: x.mean if isinstance(x, Triangular) else x).mean 
...
class SVTNN(Triangular):
    ...
    def maximum(SVTNN_list):
        max_T_mean = max(SVTNN_list, key=lambda x: x.Truth[1] if isinstance(x, SVTNN) else x).Truth[1] 
